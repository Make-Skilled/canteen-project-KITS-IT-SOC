...
const DATABASE_URL = "mongodb://127.0.0.1:17017/KITS-SOC-canteen";
...
app.post('/studentregister', async (req, res) => {
  ...
  try {
    ...
    const user = new User({ name, email, password: hashedPassword, phNo});
...
app.post('/login', async (req, res) => {
  ...
  // Send the name along with the response
...
app.post('/ownerlogin', async (req, res) => {
  ...
  const ownermail = "kitscanteen@gmail.com";
  const ownerpassword = "canteenkits@2396";
  ...
  res.json({ success: true, message: 'Login successful', ownerId: "manual_owner_id_001" });
});
...
app.post('/items', async (req, res) => {
  ...
  // Check if the createdBy ID is vali
...
app.get('/orders/:userId', async (req, res) => {
  ...
  try {
    const orders = await Order.find({ orderedBy: userId }).populate('item'); // Assuming "item" is a reference to FoodItem
    res.json(orders);
  } catch (error) {
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.get('/owneritems', async (req, res) => {
  ...
  try {
    ...
    if (!items || items.length === 0) {
      return res.json({ message: 'No food items found for this owner' });
    }
...
app.put('/items/:itemId', async (req, res) => {
  ...
  try {
    ...
    if (item.createdBy !== req.body.createdBy) {
...
app.delete('/items/:itemId', async (req, res) => {
  const { itemId } = req.params;
  const { createdBy } = req.body; // Get the ownerId from the request body
  ...
  try {
    ...
    if (item.createdBy.toString() !== createdBy) {
...
app.post('/logout', async (req, res) => {
  // For stateless applications (like JWT), there is nothing to clear on the server-side
  // The client will handle removing the token from storage or memory.
  
  // If using a session-based approach (like express-session), you would do something like:
